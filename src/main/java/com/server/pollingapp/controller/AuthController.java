...
public class AuthController {
  ...
  signUpUser(@RequestBody @Valid RegistrationRequest registrationRequest,
             BindingResult bindingResult) {
    // IF INPUT HAS ERRORS WHICH IT SHOULDN'T CAUSE FRONTEND VALIDATED JUST
    // RETURN THIS
  ...
  @PostMapping(value = "/api/v1/auth/signin",
               produces = MediaType.APPLICATION_JSON_VALUE,
               consumes = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<LoginResponse>
  signInUser(@RequestBody @Valid LoginRequest loginRequest,
             BindingResult bindingResult) {
    // IF INPUT HAS ERRORS WHICH IT SHOULDN'T CAUSE FRONTEND VALIDATED JUST
    // RETURN THIS
    if (bindingResult.hasErrors()) {
  ...
  @PutMapping(value = "/api/v1/auth/activate/{token}",
              produces = MediaType.APPLICATION_JSON_VALUE)
  public ResponseEntity<UniversalResponse>
  activateAccount(@PathVariable String token) {
