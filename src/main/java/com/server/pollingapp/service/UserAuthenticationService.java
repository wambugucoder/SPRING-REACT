 ...
 * @since 13-04-2021
 * @apiNote <p>
 *     <b> The UserAuthenticationService should only contain
 * RegisterUser(signup),LoginUser(signIn) and validate access token
 * methods).</b>
 *
 *     <li>RegisterUser-> Collects User Details,Validates them,Encrypts
 * password,Saves details to db and sends an email to activate account</li>
 *     <li>LoginUser->Collects Login Credentials,if credentials are correct it
 * generates a jwtToken for session Mgmt</li> <li>ValidateAccount-> Once
 * Registered an Email is Sent containing a link, link contains token which
 * should be validated.This is to verify that the account user is real.
 *      </li>
 *
 * </p>
 */
...
public class UserAuthenticationService {
  ...
  public UserAuthenticationService(
      @Lazy UserRepositoryImpl userRepositoryImpl,
      @Lazy BCryptPasswordEncoder bCryptPasswordEncoder,
      @Lazy AuthenticationManager authenticationManager,
      @Lazy JwtService jwtService, @Lazy EmailService emailService,
  ...
  RegisterUser(RegistrationRequest registrationRequest) {
    ...
    if (userRepository.existsByEmail(registrationRequest.getEmail())) {
      ...
      // GENERATE LOGS
      log.warn(registrationRequest.getEmail() + " "
               + "Already Exists");
      ...
      // GENERATE LOGS
      log.warn(registrationRequest.getUsername() + ""
               + "Already Exists");
      return ResponseEntity.badRequest().body(details);
    }
    // OTHERWISE OF EVERYTHING IS FINE ,ENCRYPT PASSWORD,SAVE USER AND SEND THEM
    // A CONFIRMATION EMAIL
    String password =
        bCryptPasswordEncoder.encode(registrationRequest.getPassword());

    UserModel newUser = new UserModel(registrationRequest.getUsername(),
    ...
    // GENERATE LOGS
    log.info("User with Username" + newUser.getUsername() + " "
             + "has just been registered in the system");
  ...
  public ResponseEntity<LoginResponse> LoginUser(LoginRequest loginRequest) {
    try {
      ...
      // GENERATE LOGS
      log.warn(loginRequest.getEmail() + " "
               + "Has Provided Invalid Credentials");
      // RETURN 404 ERROR
      LoginResponse loginResponse = new LoginResponse();
      loginResponse.setError(true);
  ...
  public ResponseEntity<UniversalResponse> ActivateUserAccount(String token) {
    ...
    if (user.getEnabled()) {
      ...
      // GENERATE LOG
      log.warn(email + " "
               + "Tried to access the activation token Again");
