...
export const ActivateUserAccount = (token) => (dispatch) => {
  dispatch({ type: IS_LOADING });
  axios
    .put(`/api/v1/auth/activate/${token}`)
    .then((result) => {
      dispatch({ type: ACTIVATE_USER_ACCOUNT, payload: result.data });
    })
    .catch((err) => {
      dispatch({ type: ACTIVATION_ERRORS, payload: err.response.data });
    });
};
...
export const CreateNonScheduledPoll = (userId, pollData) => (dispatch) => {
  dispatch({ type: IS_LOADING });
  axios
    .post(`/api/v1/polls/${userId}/non_scheduled_poll`, pollData)
    .then((result) => {
      dispatch({
...
export const CreateScheduledPoll = (userId, pollData) => (dispatch) => {
  ...
  axios
    ...
    .catch((err) => {
      dispatch({ type: SCHEDULED_POLL_ERRORS, payload: err.response.data });
    });
};
