...
function CreatePollDashboard() {
  ...
  useEffect(
      () => {
        if (poll.hasCreatedScheduledPoll === true ||
            poll.hasCreatedNonScheduledPoll === true) {
          message.success("Poll has Been Created Successfully")
        }
        return () => { dispatch(CleanupPoll()) };
      },
      [
        dispatch, poll.hasCreatedNonScheduledPoll, poll.hasCreatedScheduledPoll,
        poll.isLoading
      ])
  ...
  useEffect(
      ...
      [
        dispatch, error.errorHandler.message, error.hasNonScheduledPollErrors,
        error.hasScheduledPollErrors, error.isLoading
      ])
  ...
  return(
    ...
    <TabPane tab={
  <span><FormOutlined />Create Poll<
      /span>} key="1">
      <div className="poll-section-1"><NonScheduledPoll/>
      </div>
    </TabPane><Space />< TabPane tab = {
    <span><CalendarFilled />Schedule Poll<
        /span>} key="2">
    <div className="poll-section-1"><ScheduledPoll/>
    </div>
    </TabPane></Tabs>
              </Col>
    </Row>
